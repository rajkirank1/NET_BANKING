    ...
    public Exception decode(String s, Response response) {
        ...
        switch (response.status()) {
            ...
            default: {
                ...
                return new Exception("Feign client error with status: " + response.status());
            }
        }
    }
    ...
    private GlobalException extractGlobalException(Response response) {
        ...
        try {
            if (response.body() != null) {
                ...
                log.error("Feign client error response: {}", result);

                ObjectMapper mapper = new ObjectMapper();
                mapper.disable(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES);

                globalException = mapper.readValue(result, GlobalException.class);
                log.error("Parsed GlobalException: {}", globalException.toString());
            }
        } catch (IOException e) {
